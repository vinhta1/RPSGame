...
namespace JoyconUnity
{
    ...
    public class Joycon
    {
        ...
        private void ExtractImuValues(byte[] reportBuf, int n = 0)
        {
            ...
            {
                //
                acc_g[i] = Mathf.Floor(acc_r[i] * 0.00122187695f); //0.00025f originally
                gyr_g[i] = Mathf.Floor((gyr_r[i] - gyr_neutral[i]) * 0.00122187695f);
                if (Math.Abs(acc_g[i]) > Math.Abs(max[i]))
                    max[i] = acc_g[i];
                //
                acc_g.SetAxisValue(i, acc_r[i] * 0.00122187695f);
